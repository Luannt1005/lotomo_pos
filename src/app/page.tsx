      ...
      {selectedProduct && (
        <div className="fixed inset-0 z-[100] bg-black/60 backdrop-blur-sm flex items-center justify-center p-1 lg:p-4 animate-in fade-in">
           <div className="bg-white w-full max-w-[420px] rounded-2xl lg:rounded-[2.5rem] shadow-2xl overflow-hidden flex flex-col max-h-[98vh] border-[1px] lg:border-[4px] border-white animate-in zoom-in-95 duration-500">
              ...
              <div className="flex-1 p-3 lg:p-6 space-y-3 lg:space-y-6 overflow-y-auto no-scrollbar">
                 <div className="flex items-center gap-2">
                    <label className="text-[7px] font-black uppercase tracking-widest opacity-30 w-8 lg:w-12">Size</label>
                    <div className="flex-1 grid grid-cols-3 gap-1 lg:gap-2">
                        {selectedProduct.sizes.map(s => (
                            <button key={s.size} onClick={() => setSize(s.size)} className={`py-1.5 lg:py-2.5 rounded-lg border-2 font-black transition-all text-[8px] lg:text-[10px] uppercase ${size === s.size ? "bg-primary text-white border-primary shadow-md" : "bg-muted border-transparent text-muted-foreground"}`}>{s.size}</button>
                        ))}
                    </div>
                 </div>

                 <div className="flex items-center gap-2">
                    <label className="text-[7px] font-black uppercase tracking-widest opacity-30 w-8 lg:w-12">Đường</label>
                    <div className="flex-1 grid grid-cols-3 gap-1 lg:gap-2">
                        {(['0%', '50%', '100%'] as SugarLevel[]).map(s => (
                            <button key={s} onClick={() => setSugar(s)} className={`py-1.5 lg:py-2.5 rounded-lg border-2 font-black uppercase text-[8px] lg:text-[10px] tracking-widest transition-all ${sugar === s ? "bg-primary text-white border-primary shadow-sm" : "bg-muted border-transparent text-muted-foreground"}`}>{s}</button>
                        ))}
                    </div>
                 </div>

                 <div className="flex items-center gap-2">
                    <label className="text-[7px] font-black uppercase tracking-widest opacity-30 w-8 lg:w-12">Đá</label>
                    <div className="flex-1 grid grid-cols-3 gap-1 lg:gap-2">
                        {(['không đá', 'ít đá', 'bình thường'] as IceLevel[]).map(i => (
                            <button key={i} onClick={() => setIce(i)} className={`py-1.5 lg:py-2.5 rounded-lg border-2 font-black uppercase text-[8px] lg:text-[10px] tracking-widest transition-all ${ice === i ? "bg-primary text-white border-primary shadow-sm" : "bg-muted border-transparent text-muted-foreground"}`}>{i}</button>
                        ))}
                    </div>
                 </div>
                 ...
                 {selectedProduct.category === 'matcha' && (
                  <div className="flex items-center gap-2 animate-in slide-in-from-left-2">
                      <label className="text-[7px] font-black uppercase tracking-widest opacity-30 w-8 lg:w-12">Sữa</label>
                      <div className="flex-1 grid grid-cols-3 gap-1 lg:gap-2">
                          {(['sữa tươi', 'sữa Oat', 'sữa Meiji'] as MilkType[]).map(m => (
                              <button key={m} onClick={() => setMilk(m)} className={`py-1.5 lg:py-2.5 rounded-lg border-2 font-black uppercase text-[8px] lg:text-[10px] tracking-widest transition-all ${milk === m ? "bg-primary text-white border-primary shadow-sm" : "bg-muted border-transparent text-muted-foreground"}`}>
                                {m} {m !== 'sữa tươi' && "+5K"}
                              </button>
                          ))}
                      </div>
                 ...
                 <div className="space-y-1.5">
                    <label className="text-[7px] lg:text-[9px] font-black uppercase tracking-widest opacity-30">Toppings</label>
                    <div className="grid grid-cols-2 gap-1 lg:gap-2 max-h-[80px] lg:max-h-[120px] overflow-y-auto no-scrollbar pr-1 bg-[#f8f9fa] p-1.5 lg:p-2 rounded-xl border border-black/5">
                        {allToppings.map(t => (
                            <button key={t.id} onClick={() => toggleTopping(t.name)} className={`p-1.5 lg:p-3 rounded-lg lg:rounded-xl border-2 flex justify-between items-center transition-all ${selectedToppings.includes(t.name) ? "bg-primary/5 border-primary text-primary shadow-sm" : "bg-white border-transparent text-muted-foreground opacity-70"}`}>
                                <span className="font-black uppercase text-[7px] tracking-tight line-clamp-1">{t.name}</span>
                 ...
                 <div className="space-y-1.5">
                    <textarea value={note} onChange={e => setNote(e.target.value)} className="w-full bg-[#f8f9fa] border-2 border-transparent focus:border-primary/20 rounded-xl p-2 outline-none font-bold text-[10px] h-10 lg:h-14 resize-none" placeholder="Ghi chú..."></textarea>
                 </div>
              </div>
